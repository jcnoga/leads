...
function fx(name) {
  return functionsClient.httpsCallable(name);
}
...
async function handleRegister(e) {
  ...
  try {
    ...
    // O documento em users/{uid} é criado no servidor pelo gatilho onUserCreate
    // (functions/index.js) — o cliente não escreve credits/isAdmin diretamente.
    showToast("Conta criada com sucesso! Preparando sua central de leads…", "success");
  } catch (err) {
    showToast(translateFirebaseError(err), "error");
  } finally {
    setButtonLoading(btn, false);
...
function startUserSession(user) {
  ...
  unsubProfile = db.collection("users").doc(user.uid).onSnapshot(
    (snap) => {
      if (!snap.exists) return; // gatilho onUserCreate ainda não rodou; aguarda próximo snapshot
...
async function handleSearchSubmit(e) {
  ...
  try {
    const result = await fx("searchLeads")({ niche, city, state, neighborhood, limit });
    const { leads, newBalance } = result.data;
    ...
    apiStatusBox.innerHTML = `<i class="fa-solid fa-circle-check"></i> ${leads.length} lead(s) encontrados. Créditos consumidos: ${leads.length}. Saldo atual: ${newBalance}.`;
    apiStatusBox.classList.remove("hidden");
  } catch (err) {
    if (err.code === "resource-exhausted" || err.code === "functions/resource-exhausted") {
      const mockLimit = Math.min(limit, globalConfig.maxLeadsPerSearch || 150);
      const leads = generateMockLeadsClient(niche, city, state, neighborhood, mockLimit);
      currentSearchLeads = leads;
      searchIsMock = true;
      renderSearchResults(leads);
      ...
      apiStatusBox.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> ${escapeHtml(translateFirebaseError(err))}`;
      apiStatusBox.classList.remove("hidden");
    }
  } finally {
    setButtonLoading(btn, false);
...
async function handleAdminAddSelfCredits() {
  ...
  btn.disabled = true;
  try {
    const result = await fx("addSelfCredits")();
    setValidationMsg($("admin-self-msg"), `+10 créditos adicionados. Novo saldo: ${result.data.newBalance}.`, "success");
...
async function handleAdminAddCredits() {
  ...
  if (!email || !Number.isFinite(qty) || qty <= 0) {
    setValidationMsg($("admin-credits-msg"), "Preencha um e-mail e uma quantidade válida.", "error");
    return;
  }
  const btn = $("btn-admin-add-credits");
  btn.disabled = true;
  try {
    const result = await fx("addCreditsToUser")({ targetEmail: email, amount: qty });
    setValidationMsg($("admin-credits-msg"), `${qty} créditos adicionados a ${result.data.targetEmail}. Novo saldo: ${result.data.newBalance}.`, "success");
...
async function handleAdminPromote() {
  const email = $("admin-promote-email").value.trim();
  if (!email) {
  ...
  btn.disabled = true;
  try {
    const result = await fx("promoteToAdmin")({ targetEmail: email });
    setValidationMsg($("admin-promote-msg"), `✅ ${result.data.targetEmail} agora é superadministrador.`, "success");
...
async function handleAdminUpdateMaxLeads() {
  ...
  btn.disabled = true;
  try {
    await fx("updateGlobalConfig")({ maxLeadsPerSearch: value });
