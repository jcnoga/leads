...
**Correção:** `firestore.rules` agora impede explicitamente qualquer escrita do cliente nos campos `credits`, `isAdmin` e `email` do próprio documento (`request.resource.data.credits == resource.data.credits` etc.). Essas alterações só podem acontecer através das Cloud Functions, que usam o Admin SDK (que ignora as regras) depois de validar no servidor que quem está chamando é de fato administrador.

### 1.3 Causa raiz dos botões de crédito "que não funcionavam"
Esses dois pontos acima explicam o sintoma relatado no briefing: com regras de segurança corretas (como deveriam estar), a escrita direta do cliente em `credits`/`isAdmin` é negada pelo Firestore — e como o código original não tinha nenhum `try/catch` em `consumeCredits`, `addCreditsToUser` ou `addSelfCredits`, a Promise rejeitada simplesmente não fazia nada visível: o botão "+10 leads (meu saldo)" e o botão "Adicionar créditos" pareciam travados, sem nenhum erro na tela. A correção do item 1.2 resolve a causa raiz; a nova interface também exibe mensagens de erro reais (`admin-self-msg`, `admin-credits-msg`) em vez de falhar em silêncio.
...
**Correção:** `addSelfCredits` no servidor sempre adiciona exatamente 10 créditos, batendo com o texto do botão. Quantidades diferentes de crédito (para outros usuários) têm seu próprio fluxo dedicado (`addCreditsToUser`), com campo de quantidade explícito na tela.
...
**Correção:** `searchLeads` agora pagina automaticamente (parâmetro `page` da Serper, até 30 páginas), deduplicando por `cid` da empresa e, como reforço, por nome+endereço. Para assim que atingir o limite pedido (até o teto configurado pelo admin) ou quando a API parar de devolver resultados novos.
...
|---|---|
| Botões de crédito corrigidos | Cloud Functions transacionais + regras de segurança (seções 1–2 acima) |
...
| Busca de até 150 leads (configurável) | Paginação automática da Serper + campo "Limite de leads por busca" no painel de administração (1–500) |
...
- **Perfil do usuário criado pelo servidor** (gatilho `onUserCreate`), não mais pelo cliente: fecha a possibilidade de um cadastro malicioso definir `isAdmin: true` ou um saldo inicial inflado direto na criação da conta.
- **Modo demonstração mantido**, mas sem custo: quando o saldo está zerado, a busca volta a gerar leads fictícios (claramente identificados) só no navegador, sem chamar a função paga — preserva a experiência de demonstração do produto original sem repetir o problema de custo descontrolado.
